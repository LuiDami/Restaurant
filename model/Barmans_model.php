...
/**
 * Description of Barmans
 *
 * @author Luis Damian
 */
include_once 'Crud_model.php';
class Barmans extends Crud {
    private $id_barman;
    private $dni;
    private $estado;
    private $conexion;
    const TABLA=["barmans", "id_barman"]; // declaramos una constante en clase
    ...
    function __construct() {
         ...
         **/
        parent::__construct(Barmans::TABLA);
    ...
    public function actualizar($atributo, $valor) {
        ...
        try {
            $sql = "UPDATE ".Barmans::TABLA[0]." SET $atributo = '$valor' WHERE id_barman = '$this->id_barman'"; 
    ...
    public function crear() {
        ...
        // para acceder a una cosntante de la clase NombreClase::NOMBRECONSTANTE
        try {
           $sql="INSERT INTO ".Barmans::TABLA[0]." (dni, estado) VALUES ('$this->dni', '$this->estado')";
            $this->conexion->query($sql);
            $this->__set("id_barman", $this->conexion->insert_id); 
        } catch (Exception $ex) {
            echo 'error '.$ex;
        }
    ...
    /**
     * Obtiene Barmans por dni
     * @param type $dni
     */
    public function obtener_por_dni($dni){
        $sql= "SELECT * FROM ".Barmans::TABLA[0]." WHERE dni = '$dni'";
    ...
    public function obtener_trabajadores(){
        try {
            $sql= "SELECT * FROM `trabajadores` t1 INNER JOIN ".Barmans::TABLA[0]." t2 ON t1.dni = t2.dni";
            $resultado=$this->conexion->query($sql);
            //$salida = $resultado->fetch_object();
    ...
    /**
     * Obtiene Barmans con los datos de trabajadores desactivado
     */
    public function obtener_trabajadores_desactivados(){
        try {
            $sql= "SELECT * FROM `trabajadores` t1 INNER JOIN ".Barmans::TABLA[0]." t2 ON t1.dni = t2.dni WHERE t2.estado=0";
            $resultado=$this->conexion->query($sql);
            //$salida = $resultado->fetch_object();
    ...
    /**
     * Obtiene Barmans con los datos de trabajadores activados
     */
    public function obtener_trabajadores_activados(){
        try {
            $sql= "SELECT * FROM `trabajadores` t1 INNER JOIN ".Barmans::TABLA[0]." t2 ON t1.dni = t2.dni WHERE t2.estado=1";
            $resultado=$this->conexion->query($sql);
            //$salida = $resultado->fetch_object();
