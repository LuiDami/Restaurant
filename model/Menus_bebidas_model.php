...
class MenusBebidas extends Crud {
    //put your code here
    private $id_menu;
    private $id_bebida;
    private $conexion;
    const TABLA=["menu_bebida", "id_menu"];
    ...
    function __construct() {
         ...
         **/
        parent::__construct(MenusBebidas::TABLA);
    ...
    public function actualizar($atributo, $valor) {
        ...
        */ 
        ...
        try {
            $sql = "UPDATE ".MenusBebidas::TABLA[0]." SET $atributo = '$valor' WHERE id_menu = '$this->id_menu'"; 
    ...
    public function crear() {
        ...
        try {
            $sql="INSERT INTO ".MenusBebidas::TABLA[0]." (`id_menu`, `id_bebida`) VALUES ('$this->id_menu', '$this->id_bebida')";
            $this->conexion->query($sql);
     ...
     * @param type $tipo
     */
    public function obtener_bebidasMenu_por_tipo($id_menu, $tipo=""){
        $auxiliar="";
        if($tipo=="refresco_con_gas"){
            $auxiliar= "&& t1.tipo='$tipo'";
        }
        if($tipo=="refresco_sin_gas"){
            $auxiliar= "&& t1.tipo='$tipo'";
        }
        if($tipo=="zumo"){
            $auxiliar= "&& t1.tipo='$tipo'";
        ...
        try {
             $sql= "SELECT * FROM `bebidas` t1 INNER JOIN ".MenusBebidas::TABLA[0]." t2 ON t1.id_bebida = t2.id_bebida WHERE t2.id_menu=".$id_menu." ".$auxiliar;
    ...
    function borrarBebidadeMenu($id_bebida, $id_menu){
        /**
         ...
         */
        try {
            $sql="DELETE FROM ".MenusBebidas::TABLA[0]." WHERE id_bebida like '$id_bebida' && id_menu like '$id_menu'";
            $this->conexion->query($sql);
        } catch (Exception $ex) {
            echo 'error '.$ex;
        }
