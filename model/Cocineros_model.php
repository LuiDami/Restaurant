...
/**
 * Description of Cocineros
 *
 * @author Luis Damian
 */
include_once 'Crud_model.php';
class Cocineros extends Crud {
    private $id_cocinero;
    private $dni;
    private $estado;
    private $conexion;
    const TABLA=["cocineros", "id_cocinero"]; // declaramos una constante en clase
    ...
    function __construct() {
         ...
         **/
        parent::__construct(Cocineros::TABLA);
    ...
    public function actualizar($atributo, $valor) {
        ...
        try {
            $sql = "UPDATE ".Cocineros::TABLA[0]." SET $atributo = '$valor' WHERE id_cocinero = '$this->id_cocinero'"; 
    ...
    public function crear() {
        ...
        // para acceder a una cosntante de la clase NombreClase::NOMBRECONSTANTE
        try {
           $sql="INSERT INTO ".Cocineros::TABLA[0]." (dni, estado) VALUES ('$this->dni', '$this->estado')";
            $this->conexion->query($sql);
            $this->__set("id_cocinero", $this->conexion->insert_id); 
        } catch (Exception $ex) {
            echo 'error '.$ex;
        }
    ...
    /**
     * Obtiene Cocineros por dni
     * @param type $dni
     */
    public function obtener_por_dni($dni){
        $sql= "SELECT * FROM ".Cocineros::TABLA[0]." WHERE dni = '$dni'";
    ...
    public function obtener_trabajadores(){
        try {
            $sql= "SELECT * FROM `trabajadores` t1 INNER JOIN ".Cocineros::TABLA[0]." t2 ON t1.dni = t2.dni";
            $resultado=$this->conexion->query($sql);
            //$salida = $resultado->fetch_object();
    ...
    /**
     * Obtiene Cocineros con los datos de trabajadores desactivado
     */
    public function obtener_trabajadores_desactivados(){
        try {
            $sql= "SELECT * FROM `trabajadores` t1 INNER JOIN ".Cocineros::TABLA[0]." t2 ON t1.dni = t2.dni WHERE t2.estado=0";
            $resultado=$this->conexion->query($sql);
            //$salida = $resultado->fetch_object();
    ...
    /**
     * Obtiene Cocineros con los datos de trabajadores activados
     */
