...
class Camareros extends Crud {
    private $id_camarero;
    private $dni;
    private $estado;
    private $conexion;
    const TABLA=["camareros", "id_camarero"]; // declaramos una constante en clase
    ...
    function __construct() {
         ...
         **/
        parent::__construct(Camareros::TABLA);
    ...
    public function actualizar($atributo, $valor) {
        ...
        try {
            $sql = "UPDATE ".Camareros::TABLA[0]." SET $atributo = '$valor' WHERE id_camarero = '$this->id_camarero'"; 
    ...
    public function crear() {
        ...
        // para acceder a una cosntante de la clase NombreClase::NOMBRECONSTANTE
        try {
           $sql="INSERT INTO ".Camareros::TABLA[0]." (dni, estado) VALUES ('$this->dni', '$this->estado')";
            $this->conexion->query($sql);
            $this->__set("id_camarero", $this->conexion->insert_id); 
        } catch (Exception $ex) {
            echo 'error '.$ex;
        }
    ...
    /**
     * Obtiene Camareros por dni
     * @param type $dni
     */
    public function obtener_por_dni($dni){
        $sql= "SELECT * FROM ".Camareros::TABLA[0]." WHERE dni = '$dni'";
    ...
    public function obtener_trabajadores(){
        try {
            $sql= "SELECT * FROM `trabajadores` t1 INNER JOIN ".Camareros::TABLA[0]." t2 ON t1.dni = t2.dni";
            $resultado=$this->conexion->query($sql);
            //$salida = $resultado->fetch_object();
    ...
    /**
     * Obtiene Camareros con los datos de trabajadores desactivado
     */
    public function obtener_trabajadores_desactivados(){
        try {
            $sql= "SELECT * FROM `trabajadores` t1 INNER JOIN ".Camareros::TABLA[0]." t2 ON t1.dni = t2.dni WHERE t2.estado=0";
            $resultado=$this->conexion->query($sql);
            //$salida = $resultado->fetch_object();
    ...
    /**
     * Obtiene Camareros con los datos de trabajadores activados
     */
    public function obtener_trabajadores_activados(){
        try {
            $sql= "SELECT * FROM `trabajadores` t1 INNER JOIN ".Camareros::TABLA[0]." t2 ON t1.dni = t2.dni WHERE t2.estado=1";
            $resultado=$this->conexion->query($sql);
            //$salida = $resultado->fetch_object();
