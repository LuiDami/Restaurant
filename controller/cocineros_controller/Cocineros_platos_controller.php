...
<?php 
     ...
     */
    if (isset($_POST["cocinero"]["platos"]["crear"])) {
        if($_POST["cocinero"]["platos"]["crear"]=="Nuevo Plato"){
        $_SESSION["login"]["crear"]=true;   
        }
    }
    
    if ((isset($_POST["cocinero"]["platos"]["crear"]))&&$_SESSION["login"]["crear"]) {
        /**----------------------------------------------------------crear--------------------------------------
         * para crear mas platos
         */
        $mensajeCreacion="";
        if (isset($_POST["cocinero"]["platos"]["crear"]["submit"])) {
            ...
            if ($tipo_foto==""||$tipo_foto=="image/jpeg") {
                ...
                $plato->nombre=$_POST["cocinero"]["platos"]["crear"]["nombre"];
                $plato->descripcion=$_POST["cocinero"]["platos"]["crear"]["descripcion"];
                $plato->tipo=$_POST["cocinero"]["platos"]["crear"]["tipo"];
                $plato->estado=$_POST["cocinero"]["platos"]["crear"]["estado"];
                $plato->foto=$destino_foto;
                $plato->receta=$_POST["cocinero"]["platos"]["crear"]["receta"];
                $plato->crear();
                ...
                include_once '././view/cocineros_view/Cocineros_platos_view.php'; 
                ...
                include_once '././view/cocineros_view/Cocineros_platos_crear_view.php'; 
            }
    ...
    } elseif (isset($_POST["cocinero"]["platos"]["editar"])) {
        /**------------------------------------------------------editar-----------------------------------------------------
         * para editar un plato
         * obtenemos el id por post
         */
        $id=$_POST["cocinero"]["platos"]["editar"]["id"];

        if (isset($_POST["cocinero"]["platos"]["editar"]["submit"])) {
            /**
            ...
            */
            $datosCambio=$_POST["cocinero"]["platos"]["editar"]["datos"];
            ...
            include_once '././view/cocineros_view/Cocineros_platos_ver_view.php'; 
        }else{
            /**
             * creamos las variables necesarias para imprimir la vista
             * en el caso de que no tuviese foto se pondra la imagen por defecto
